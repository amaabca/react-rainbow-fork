...
function PresenceMap(props) {
    ...
    var Component = (0, _react.useCallback)(
        (0, _reactAsyncScriptLoader['default'])(''.concat(googleMapApiUrl, '?key=').concat(apiKey))(
            _component['default'],
        ),
        [apiKey],
    );
...
PresenceMap.propTypes = {
    ...
    markers: _propTypes['default'].arrayOf(
    ...
    center: _propTypes['default'].oneOfType([
        _propTypes['default'].shape({
            lat: _propTypes['default'].number,
            lng: _propTypes['default'].number,
        }),
